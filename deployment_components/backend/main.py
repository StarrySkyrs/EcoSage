...
async def lifespan(app: FastAPI) -> None:
     #startup
     server_resources['qna_obj'], server_resources['qna_filename'],server_resources['qna_filepath'] = qna_create('../../data/waste_info')
     #delete any .jpg in ../../data/input_images
     for image in Path('../../data/input_images').iterdir():
        if image.suffix == ".jpg":
            os.remove(image)
        else: 
            pass
     ...
     for image in Path('../../data/segmented_images').iterdir():
        if image.suffix == ".png":
            os.remove(image)
        else: 
            pass
...
#     with open(f"./data/input_images/{file.filename}", "wb") as buffer:
...
class QnAdoc:
    ...
    def __init__(self,file_path:str)->None:
        ...
        self.chain = ChainCreator(deployment="davinci003-qa",docsearch=self.docsearch)
...
def run_pipeline(): 
    ...
    segment_images_dir = "../../data/segmented_images"
    image_path = ''
...
def get_label(image_path:str)->str:
    """Get the image label from the image using the HuggingFace API
